function [error_classifier_current,...
          beta_0_current] = ...
    beta_metric_learning_binary_classification( dataset_i,n_beta,class_test, ...
    feature_train_test, ...
    initial_label_index, ...
    class_train_test, ...
    classifier_i)
...
%         x_valid(~initial_label_index)=label_svm_rbf;
    ...
    if flag==0
        disp(['obj before bo : ' num2str(x_valid'*cL*x_valid) ' | beta: ' num2str(beta_0) ' | dataset: ' num2str(dataset_i)]);
        disp(['error before bo : ' num2str(error_classifier) ' | beta: ' num2str(beta_0) ' | dataset: ' num2str(dataset_i)]);
%         current_obj=x'*cL*x;
        current_obj=current_H;
        error_classifier_0=error_classifier;
        x_valid_previous=x_valid;
    else
        disp(['obj after bo : ' num2str(x_valid'*cL*x_valid) ' | beta: ' num2str(beta_0) ' | dataset: ' num2str(dataset_i)]);
        disp(['error after bo : ' num2str(error_classifier) ' | beta: ' num2str(beta_0) ' | dataset: ' num2str(dataset_i)]);
%         current_obj=x'*cL*x;
        current_obj=current_H;
