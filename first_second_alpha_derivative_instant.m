...
% end
    ss_FW=-sum(sum(lambda_FW_alpha,2)./(sum(lambda_FW_alpha_0,2)+beta_p_const));
    sss_FW=sum(sum(lambda_FW_alpha,2).^2./(sum(lambda_FW_alpha_0,2)+beta_p_const).^2);
f_alpha_first=ldp*ss_FW;
