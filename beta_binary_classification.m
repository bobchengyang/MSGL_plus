...
tol_compare=-Inf;
beta_0_current=ones(1,n_beta); % initial beta's
...
while tol_compare<0
    ...
    if flag==1
        ...
        x_current=x_valid;
        beta_0_current=beta_0_can;
    end
    
    eig_tol=1e-8;
    if flag==0
        lambda=sum(repmat(beta_0_current,[n_sample_full 1]).*l_matrix,2)+eig_tol;
    else
...
%         x_valid(~initial_label_index)=label_svm_rbf;
    ...
    [ x_valid,n_class ] = class_num_to_vec( x_valid,n_sample_full );
    
    if flag==0
        disp(['obj before bo : ' num2str(trace(x_valid'*cL*x_valid)) '/' num2str(trace(x_valid'*cL*x_valid)-sum(log(lambda))) ' | beta: ' num2str(beta_0_current) ' | dataset: ' num2str(dataset_i)]);
        disp(['error before bo : ' num2str(error_classifier) ' | beta: ' num2str(beta_0_current) ' | dataset: ' num2str(dataset_i)]);
        current_obj=trace(x_valid'*cL*x_valid)-sum(log(lambda));
%         current_obj=current_H;
        beta_0_can=beta_0_current;
        error_classifier_0=error_classifier;
        x_valid_previous=x_valid;
    else
        disp(['obj after bo : ' num2str(trace(x_valid'*cL*x_valid)) '/' num2str(trace(x_valid'*cL*x_valid)-sum(log(lambda))) ' | beta: ' num2str(beta_0_test) ' | dataset: ' num2str(dataset_i)]);
        disp(['error after bo : ' num2str(error_classifier) ' | beta: ' num2str(beta_0_test) ' | dataset: ' num2str(dataset_i)]);
        current_obj=trace(x_valid'*cL*x_valid)-sum(log(lambda));
%         current_obj=current_H;
        tol_compare=current_obj-previous_obj;
        beta_0_can=beta_0_test;
    ...
    [ beta_0_test ] = beta_optimization_LP(n_beta,zeros(1,n_beta),v,l_matrix,x_valid,n_sample_full,eig_tol,tol_set,mo );
