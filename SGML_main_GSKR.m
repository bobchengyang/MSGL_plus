%=================================================================
% Signed Graph Metric Learing (SGML) via Gershgorin Disc Alignment
% **SGML main function
%
% author: Cheng Yang
% email me any questions: cheng.yang@ieee.org
% date: June 16th, 2020
% please kindly cite the paper: 
% ['Signed Graph Metric Learning via Gershgorin Disc Alignment', 
% Cheng Yang, Gene Cheung, Wei Hu, 
% https://128.84.21.199/abs/2006.08816]
%=================================================================
function [M] = SGML_main_GSKR(M,...
    data_feature,...
    data_label,...
    n_sample,...
    n_feature,...
    tol_NR,...
    tol_GD,...
    rho,...
    max_iter,...
    GS_or_NR,...
    tol_golden_search,...
    options,...
    FW_dia_offdia_tol,...
    FW_full_tol,...
    C,...
    nv_od,...
    nv_full,...
    zz,...
    dia_idx,...
    num_list,...
    league_vec,...
    bins,...
    fv1,...
    scaled_M,...
    scaled_factors,...
    LP_A_sparse_i,...
    LP_A_sparse_j,...
    LP_A_sparse_s,...
    LP_b,...
    LP_lb,...
    LP_ub,...
    n_beta,...
    beta_0_current,...
    diag_idx3_full)

%%=====objective function used here: graph Laplacian regularizer(GLR)======
[c] = get_graph_Laplacian_variables_ready(data_feature,data_label,n_sample,n_feature); % replace this if you need to run SGML on a different objective function
[ L,y ] = graph_Laplacian( n_sample, c, M,data_label ); % replace this if you need to run SGML on a different objective function
cL=0;
for i_power=1:n_beta
    cL=cL+beta_0_current(i_power)*L^i_power;
end
initial_objective = data_label' * cL * data_label; % replace this if you need to run SGML on a different objective function
%%=========================================================================

disp(['initial objective value = ' num2str(initial_objective)]);

obj_f=initial_objective;

for node_number = 1:n_feature % optimzing the diagonals and one row/column of off-diagonals

    remaining_idx=1:n_feature;
    remaining_idx(node_number)=[];
    
    % compute gradient 
    % replace the following gradient function for GLR if you are using a different
    % objective function.
    [ G ] = compute_gradient( ...
        n_sample, ...
        n_feature, ...
        c, ...
        M, ...
        y, ...
        nv_od, ...
        node_number, ...
        remaining_idx,...
        beta_0_current,...
        diag_idx3_full);

    %% BLUE or RED starts
    Ms_off_diagonal = scaled_M(remaining_idx,remaining_idx); % the submatrix M22 after removing the node_number-th row/column of M.
    scaled_factors_h = scaled_factors(node_number,remaining_idx); % the scaled factors of M21
    
    %% try BLUE league on NODE node_number
    league_vec_temp = league_vec;
    league_vec_temp(node_number) = 1; % BLUE is 1
    league_vec_remaining = league_vec_temp;
    league_vec_remaining(node_number) = [];
    
    %% FW iterations BLUE
    [M_blue,...
        scaled_M_blue,...
        scaled_factors_blue,...
        fv1_blue,...
        min_obj_blue,...
        bins_blue,...
        num_list_blue,...
        LP_A_sparse_i,...
        LP_A_sparse_j,...
        LP_A_sparse_s,...
        LP_b,...
        LP_lb,...
        LP_ub,...
        LP_Aeq,...
        LP_beq,...
        zero_mask,...
        scaler_v,...
        remaining_idx,...
        lu_bound_idx] = FW_iterations_blue(1,...
        league_vec,...
        league_vec_temp,...
        league_vec_remaining,...
        scaled_factors_h,...
        Ms_off_diagonal,...
        n_feature,...
        G,...
        M,...
        node_number,...
        remaining_idx,...
        fv1,...
        rho,...
        C,...
        scaled_M,...
        scaled_factors,...
        bins,...
        obj_f,...
        tol_golden_search,...
        nv_od,...
        zz,...
        num_list,...
        n_sample,...
        c,...
        y,...
        data_label,...
        options,...
        LP_A_sparse_i,...
        LP_A_sparse_j,...
        LP_A_sparse_s,...
        LP_b,...
        LP_lb,...
        LP_ub,...
        dia_idx,...
        tol_NR,...
        tol_GD,...
        GS_or_NR,...
        max_iter,...
        FW_dia_offdia_tol,...
        n_beta,...
        beta_0_current,...
        diag_idx3_full);
    
    %% try RED league on NODE node_number
    league_vec_temp(node_number) = -1; % RED is -1
    
    %% FW iterations RED
    [M_red,...
        scaled_M_red,...
        scaled_factors_red,...
        fv1_red,...
        min_obj_red,...
        bins_red,...
        num_list_red] = FW_iterations_red(-1,...
        league_vec,...
        league_vec_temp,...
        league_vec_remaining,...
        scaled_factors_h,...
        n_feature,...
        G,...
        M,...
        node_number,...
        remaining_idx,...
        fv1,...
        rho,...
        C,...
        scaled_M,...
        scaled_factors,...
        bins,...
        obj_f,...
        tol_golden_search,...
        nv_od,...
        zz,...
        num_list,...
        n_sample,...
        c,...
        y,...
        data_label,...
        LP_A_sparse_i,...
        LP_A_sparse_j,...
        LP_A_sparse_s,...
        LP_b,...
        LP_lb,...
        LP_ub,...
        LP_Aeq,...
        LP_beq,...
        zero_mask,...
        scaler_v,...
        lu_bound_idx,...
        options,...
        dia_idx,...
        tol_NR,...
        tol_GD,...
        GS_or_NR,...
        max_iter,...
        FW_dia_offdia_tol,...
        n_beta,...
        beta_0_current,...
        diag_idx3_full);
    
    if min_obj_red < min_obj_blue
        M = M_red;
        scaled_M = scaled_M_red;
        scaled_factors = scaled_factors_red;
        fv1 = fv1_red;
        league_vec(node_number) = -1;
        bins = bins_red;
        num_list=num_list_red;
        obj_f=min_obj_red;
    end
    if min_obj_red > min_obj_blue
        M = M_blue;
        scaled_M = scaled_M_blue;
        scaled_factors = scaled_factors_blue;
        fv1 = fv1_blue;
        league_vec(node_number) = 1;
        bins = bins_blue;
        num_list=num_list_blue;
        obj_f=min_obj_blue;
    end
    [ ~,y ] = graph_Laplacian( n_sample, c, M, data_label ); % replace this if you need to run SGML on a different objective function
    %% BLUE or RED ends 
end

% compute gradient
% replace the following gradient function for GLR if you are using a different
% objective function.
[ G ] = compute_gradient( ...
    n_sample, ...
    n_feature, ...
    c, ...
    M, ...
    y, ...
    nv_full, ...
    0, ...
    0,...
    beta_0_current,...
    diag_idx3_full);

[M] = FW_iterations_full_M(league_vec,...
    scaled_factors,...
    n_feature,...
    G,...
    M,...
    rho,...
    tol_golden_search,...
    obj_f,...
    C,...
    nv_full,...
    zz,...
    n_sample,...
    c,...
    y,...
    data_label,...
    options,...
    dia_idx,...
    tol_NR,...
    tol_GD,...
    GS_or_NR,...
    max_iter,...
    FW_full_tol,...
    n_beta,...
    beta_0_current,...
    diag_idx3_full);

[ L,y ] = graph_Laplacian( n_sample, c, M, data_label );% replace this if you need to run SGML on a different objective function
cL=0;
for i_power=1:n_beta
    cL=cL+beta_0_current(i_power)*L^i_power;
end
current_objective = data_label' * cL * data_label;% replace this if you need to run SGML on a different objective function
disp(['converged objective value = ' num2str(current_objective)]);
disp(['minimal eigenvalue of M = ' num2str(min(eig(M)))]);
end