function [M,method_time] = ...
    SGML_binary_classification( ...
    feature_train_test, ...
    initial_label_index, ...
    class_train_test,...
    dataset_i)
...
% % % % n_sample_test=n_sample_full-length(find(initial_label_index));
...
% % % % error_classifier = size(find(diff_label~=0),1)*size(find(diff_label~=0),2)/size(n_sample_test,1);
% % % % 
...
%     diag_idx3_diaoffdia,...
%     diag_idx3_dia,...
%     diag_idx3_offdia ] = diag_idx( n_sample, n_feature );
