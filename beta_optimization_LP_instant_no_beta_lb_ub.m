function [ beta_0 ] = beta_optimization_LP_instant_no_beta_lb_ub( ldp,n_beta,beta_0,v,l_matrix,data_label,n_sample,eig_tol,tol_set,mo,L )
...
%     lambda=lambda/max(lambda); % not used in ICASSP submission
    ...
    LP_Aeq=[];
    for i=1:n_beta
        LP_Aeq=[LP_Aeq trace(data_label'*(L^i)*data_label)];
    end    
    LP_beq=n_sample;
    
% % % %     %====
% % % %     cvx_begin
% % % %     variable x_beta(1,n_beta)
% % % %     lambda_0=repmat(x_beta,[n_sample 1]).*l_matrix;
% % % %     lambda=sum(lambda_0,2)+eig_tol;
% % % %     minimize(-ldp*sum(log(lambda)))
% % % %     subject to 
% % % %         for iii=1:n_sample
% % % %             sum(lambda_0(iii,:))>=0;
% % % %         end
...
%     options = optimoptions('linprog','Algorithm','interior-point'); % linear program (LP) setting for Frank-Wolfe algorithm
    ...
    f=-gradient_0;
    FW_beta_direction = linprog(f,LP_A,LP_b,LP_Aeq,LP_beq,[],[],options); %LP
    ...
    disp(FW_beta_direction);
    % lll=zeros(length(0:0.01:1),1);
    % counter=0;
    % for value_try=0:0.01:1
    %     counter=counter+1;
    % lll(counter)=log_marginal_likelihood_instant(data_label,beta_0+value_try*(FW_beta_direction-beta_0),v,l_matrix,eig_tol,n_sample); % log marginal likelihood
    % end
...
% %         counter=0;
% %         for iiiii=0.01:0.01:1
% %             counter=counter+1;
% %             beta_011111=beta_0+iiiii*(FW_beta_direction-beta_0);
% %             [obj_second_round,obj_term1]=log_marginal_likelihood_instant(ldp,data_label,beta_011111,v,l_matrix,eig_tol,n_sample);
...
%         disp(['iter: ' num2str(iter) ' | beta: ' num2str(beta_0) ' | obj: ' num2str(obj_first_round) '/' num2str(obj_term1) ' | gradient: ' num2str(gradient_0) ' | step_size: ' num2str(alpha_0)]);
        disp(['iter: ' num2str(iter) ' | beta: ' num2str(beta_0) ' | obj: ' num2str(obj_first_round) '/' num2str(obj_term1) ' | step_size: ' num2str(alpha_0)]);
